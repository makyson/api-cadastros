...
class MySQL
{
     ...
     * @return string
     */
    public function delete($tabela, $id, $coluna)
    {
        $consultaDelete = 'UPDATE ' . $tabela . ' SET delete_at = 1 WHERE Cod'.$coluna.' = :id';
        if ($tabela && $id) {
    ...
    {
        if ($tabela) {
            $consulta = 'SELECT * FROM ' . $tabela . ' LIMIT 50';
     ...
     * @return mixed
     */
    public function getOneByKey($tabela, $id, $coluna)
    {
        if ($tabela && $id) {
            $consulta = 'SELECT * FROM ' . $tabela . ' WHERE Cod'.$coluna.' = :id';
